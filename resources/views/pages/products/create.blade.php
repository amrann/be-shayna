    ...
    <div class="card-body card-block">
      <form action="{{ route('products.store') }}" method="POST">
        ...
        <div class="form-group">
          ...
          {{-- {{ old('type') }} => berfungsi untuk tidak menghapus inputan ketika validasinya kena error --}}
          @error('type') 
            <div class="text-muted">{{ $message }}</div> 
          @enderror
        </div>
        <div class="form-group">
          ...
          <textarea name="description" class="inickeditor form-control @error('description') is-invalid @enderror">{{ old('description')}}</textarea>
          @error('description') 
            <div class="text-muted">{{ $message }}</div> 
          @enderror
        </div>
